...
void HAL_SAI_ErrorCallback(SAI_HandleTypeDef *hsai)
{
  ...
  audio_in_state = HAL_SAI_GetState(&hsai_BlockB2);
...
__weak void BSP_AUDIO_OUT_MspInit(SAI_HandleTypeDef *hsai, void *Params)
{
  ...
  /* CODEC_SAI pins configuration: FS, SCK, MCK and SD pins ------------------*/
    /**SAI2_A_Block_A GPIO Configuration
      ...
      PI6     ------> SAI2_SD_A
      */
  GPIO_InitStruct.Pin = AUDIO_OUT_SAIx_FS_PIN | AUDIO_OUT_SAIx_SCK_PIN
            | AUDIO_OUT_SAIx_SD_PIN | AUDIO_OUT_SAIx_MCLK_PIN;
...
//    HAL_NVIC_EnableIRQ(SAI2_IRQn);
    ...
    HAL_NVIC_SetPriority(DMA2_Stream4_IRQn, 0xE, 0);
...
uint8_t BSP_AUDIO_IN_InitEx(uint16_t InputDevice, uint32_t AudioFreq, uint32_t BitRes, uint32_t ChnlNbr)
{
  ...
  else
  {
    ...
    /* PLL clock is set depending on the AudioFreq (44.1khz vs 48khz groups) */
    BSP_AUDIO_OUT_ClockConfig(&hsai_BlockB2, AudioFreq, NULL); /* Clock config is shared between AUDIO IN and OUT */

    /* SAI data transfer preparation:
    Prepare the Media to be used for the audio transfer from SAI peripheral to memory */
    hsai_BlockB2.Instance = SAI2_Block_B;
    if(HAL_SAI_GetState(&hsai_BlockB2) == HAL_SAI_STATE_RESET)
    {
      /* Init the SAI MSP: this __weak function can be redefined by the application*/
      BSP_AUDIO_OUT_MspInit(&hsai_BlockB2, NULL);  /* Initialize GPIOs for SAI2 block A Master signals */
      BSP_AUDIO_IN_MspInit(&hsai_BlockB2, NULL);
    }
...
void BSP_AUDIO_IN_DeInit(void)
{
  SAIx_In_DeInit();
  /* DeInit the SAI MSP : this __weak function can be rewritten by the application */
  BSP_AUDIO_IN_MspDeInit(&hsai_BlockB2, NULL);
}
...
__weak void BSP_AUDIO_IN_MspInit(SAI_HandleTypeDef *hsai, void *Params)
{
  ...
  /* CODEC_SAI pin configuration: SD pin */
    ...
    GPIO_InitStruct.Pin = AUDIO_IN_SAIx_SD_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF10_SAI2;
    HAL_GPIO_Init(AUDIO_IN_SAIx_SD_GPIO_PORT, &GPIO_InitStruct);
...
//    HAL_NVIC_EnableIRQ(SAI2_IRQn);
    ...
    HAL_NVIC_SetPriority(DMA2_Stream7_IRQn, 0xF, 0);
...
static void SAIx_In_Init(uint32_t SaiOutMode, uint32_t SlotActive, uint32_t AudioFreq)
{
  ...
  /* Initialize the hsai_BlockB2 Instance parameter */
  hsai_BlockB2.Instance = SAI2_Block_B;
  ...
  LSBFirst: Disabled
  DataSize: 16 */
  hsai_BlockB2.Init.AudioFrequency = AudioFreq;
  hsai_BlockB2.Init.AudioMode = SAI_MODESLAVE_RX;
  hsai_BlockB2.Init.NoDivider = SAI_MASTERDIVIDER_ENABLED;
  ...
  hsai_BlockB2.SlotInit.SlotActive = SlotActive;
